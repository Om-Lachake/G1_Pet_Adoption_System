...
import { HousePlug, LogOut, Menu, ShoppingCart, UserCog,PawPrint } from "lucide-react";
import {
  Link,
  useLocation,
  useNavigate,
    ...
    <nav className="flex flex-col mb-3 lg:mb-0 lg:items-center gap-6 lg:flex-row">
      {PetViewHeaderMenuItems.map((menuItem) => (
        <Link
          ...
          className="text-sm font-medium cursor-pointer text-[#1a79af]"
          key={menuItem.id}
          to={menuItem.path}
        >
          {menuItem.label}
...
function HeaderRightContent(){
  ...
  return (<div className="flex lg:items-center lg:flex-row flex-col gap-4" >
    <span className="flex items-center justify-start bg-white lg:bg-[#F7F7F7] w-6 p-1 ">
      <ShoppingCart className="text-[#1a79af]" />
    ...
    <DropdownMenu>
        ...
        <DropdownMenuContent side="right" className="w-56 text-[#1a79af]">
